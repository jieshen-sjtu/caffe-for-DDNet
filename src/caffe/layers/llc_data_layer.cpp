...
namespace caffe
{
  ...
  {
    CHECK(layer_pointer);
    LLCDataLayer<Dtype>* layer =
        reinterpret_cast<LLCDataLayer<Dtype>*>(layer_pointer);
    CHECK(layer);
    Datum datum;
                                  ...
                                  vector<Blob<Dtype>*>* top)
  {
    ...
    CHECK_EQ(top->size(), 4) << "LLCData Layer takes four blobs as output: "
    << "data, llc code, patch pos, label";
                                        ...
                                        vector<Blob<Dtype>*>* top)
  {
    ...
    memcpy((*top)[3]->mutable_cpu_data(), prefetch_label_->cpu_data(),
                                        ...
                                        vector<Blob<Dtype>*>* top)
  {
    ...
    CUDA_CHECK(
        cudaMemcpy((*top)[2]->mutable_gpu_data(),
                   ...
                   cudaMemcpyHostToDevice));
    CUDA_CHECK(
        cudaMemcpy((*top)[3]->mutable_gpu_data(), prefetch_label_->cpu_data(),
                   sizeof(Dtype) * prefetch_label_->count(),
                   cudaMemcpyHostToDevice));
    // Start a new prefetch thread
    CHECK(!pthread_create(&thread_, NULL, LLCDataLayerPrefetch<Dtype>,
