...
namespace caffe
{
  ...
  void ShiftLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
                                vector<Blob<Dtype>*>* top)
  {
    CHECK_EQ(bottom.size(), 1)<< "Shift Layer takes a single blob as input.";
  ...
  template <typename Dtype>
  __global__ void kernel_shift_div(const int num, const int dim,
...
template<typename Dtype>
Dtype ShiftLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
template<typename Dtype>
Dtype ShiftLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
                                      const bool propagate_down,
                                      vector<Blob<Dtype>*>* bottom)
{
