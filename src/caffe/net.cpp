...
>>>>>>> 0a20e498651ba1aa5e4ba3a742f39637ef9451be
  ...
  void Net<Dtype>::Init(const NetParameter& param)
  {
    ...
    LOG(INFO)<< "param.input_size: " << param.input_size() << " param.input_dim_size: " << param.input_dim_size();
    // set the input blobs
    for (int i = 0; i < param.input_size(); ++i)
    {
      const string& blob_name = param.input(i);
      shared_ptr<Blob<Dtype> > blob_pointer(
          new Blob<Dtype>(param.input_dim(i * 4), param.input_dim(i * 4 + 1),
                          param.input_dim(i * 4 + 2),
                          param.input_dim(i * 4 + 3)));
      blobs_.push_back(blob_pointer);
      blob_names_.push_back(blob_name);
    ...
    for (int i = 0; i < param.layers_size(); ++i)
    {
      ...
      layers_.push_back(
          shared_ptr<Layer<Dtype> >(GetLayer<Dtype>(layer_param)));
      layer_names_.push_back(layer_param.name());
      bool need_backward = param.force_backward();
      LOG(INFO)<< "Creating Layer " << layer_param.name() << " force bp: " << need_backward;
      ...
      LOG(INFO)<< "blobs size: " << layers_[i]->blobs().size();
      if (blobs_lr_size)
      {
  ...
  void Net<Dtype>::GetLearningRateAndWeightDecay()
  {
    ...
    for (int i = 0; i < layers_.size(); ++i)
    {
      ...
      // push the learning rate multipliers
  ...
  Dtype Net<Dtype>::Backward()
  {
    ...
    for (int i = layers_.size() - 1; i >= 0; --i)
    {
      ...
      {
        Dtype layer_loss = layers_[i]->Backward(top_vecs_[i], true,
                                                &bottom_vecs_[i]);
        loss += layer_loss;
      }
    }
