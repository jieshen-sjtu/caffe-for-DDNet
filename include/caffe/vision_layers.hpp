...
// to generate probability outputs.
  ...
  public:
    explicit LLCDataLayer(const LayerParameter& param)
        : Layer<Dtype>(param), datum_channels_(0), datum_height_(0),
            datum_width_(0), datum_size_(0), thread_(0),
            llc_dim_(0)
    {
    }
...
/*
 template<typename Dtype>
 class LLCSoftmaxLayer: public Layer<Dtype>
 {
 public:
 explicit LLCSoftmaxLayer(const LayerParameter& param)
 : Layer<Dtype>(param)
 {
 }
 virtual void SetUp(const vector<Blob<Dtype>*>& bottom,
 ...
 template<typename Dtype>
 class LLCSoftmaxWithLossLayer: public Layer<Dtype>
 {
 public:
 explicit LLCSoftmaxWithLossLayer(const LayerParameter& param)
 : Layer<Dtype>(param),
 llc_softmax_layer_(new LLCSoftmaxLayer<Dtype>(param))
 {
 }
 virtual void SetUp(const vector<Blob<Dtype>*>& bottom,
 vector<Blob<Dtype>*>* top);
 ...
 shared_ptr<LLCSoftmaxLayer<Dtype> > llc_softmax_layer_;
 // prob stores the output probability of the layer.
 Blob<Dtype> prob_;
 // Vector holders to call the underlying softmax layer forward and backward.
 vector<Blob<Dtype>*> llc_softmax_bottom_vec_;
 vector<Blob<Dtype>*> llc_softmax_top_vec_;
 };
 */
}  // namespace caffe
